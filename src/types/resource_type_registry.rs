...
impl ResourceTypeRegistry {
    ...
    pub async fn build_from_schemas(schemas: &[Arc<FhirSchema>]) -> Result<Self> {
        ...
        for schema in schemas {
            let type_name = schema.title.as_ref().unwrap_or(&schema.name.as_ref().unwrap_or(&"Unknown".to_string()).clone()).clone();
            ...
            Self::process_schema_for_choice_types(schema, &choice_type_expansions, &choice_base_paths);
    ...
    fn is_primitive_schema(schema: &FhirSchema) -> bool {
        ...
        matches!(schema.schema_type.as_str(), "string" | "number" | "boolean" | "integer")
    }
...
mod tests {
    use super::*;
    use crate::types::{FhirTypeDefinitions, FhirSchema};
    
    fn create_test_schema(title: &str, schema_type: Option<&str>, description: Option<&str>) -> Arc<FhirSchema> {
        use std::collections::HashMap;
        
        Arc::new(FhirSchema {
            schema_version: Some("https://json-schema.org/draft/2020-12/schema".to_string()),
            url: Some(url::Url::parse(&format!("http://hl7.org/fhir/StructureDefinition/{}", title)).unwrap()),
    ...
    async fn test_build_from_resource_schemas() {
        ...
        let registry = ResourceTypeRegistry::build_from_schemas(&schemas).await.unwrap();
    ...
    async fn test_resource_type_enumeration() {
        ...
        let resource_types: Vec<String> = registry.get_all_resource_types().iter().cloned().collect();
    ...
    async fn test_performance_characteristics() {
        ...
        assert!(lookup_time.as_millis() < 10, "Lookups took too long: {:?}", lookup_time);
        
        // Registry building should be reasonable (under 100ms for 1000 schemas)
        let start = std::time::Instant::now();
        let _registry2 = ResourceTypeRegistry::build_from_schemas(&schemas).await.unwrap();
        let build_time = start.elapsed();
        
        assert!(build_time.as_millis() < 100, "Building took too long: {:?}", build_time);
    }
}