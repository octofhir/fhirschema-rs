...
/// Generate a fingerprint from FHIR StructureDefinition resources  
pub fn generate_fhir_package_fingerprint(
    package_id: &str,
    package_version: &str,
    structure_definitions: &[serde_json::Value],
) -> crate::Result<PackageFingerprint> {
    // Sort structure definitions by URL for consistent hashing
    let mut sorted_defs = structure_definitions.to_vec();
    ...
    let serialized = serde_json::to_vec(&sorted_defs)
        .map_err(|e| crate::FhirSchemaError::serialization_error(&e.to_string()))?;

    let mut fingerprint = generate_package_fingerprint(package_id, package_version, &serialized);

    // Add metadata about the package
    fingerprint.metadata.insert(
        "structure_definitions_count".to_string(),
        structure_definitions.len().to_string(),
    );

    fingerprint.metadata.insert(
        "generation_source".to_string(),
        "fhir_structure_definitions".to_string(),
    );

    Ok(fingerprint)
}

/// Generate a fingerprint from compiled schemas
pub fn generate_schema_fingerprint(
    package_id: &str,
    package_version: &str,
    schemas: &[crate::types::FhirSchema],
) -> crate::Result<PackageFingerprint> {
    // Sort schemas by ID for consistent hashing
    ...
    // Serialize schemas with JSON for compatibility
    let serialized = serde_json::to_vec(&sorted_schemas)
        .map_err(|e| crate::FhirSchemaError::serialization_error(&e.to_string()))?;

    let mut fingerprint = generate_package_fingerprint(package_id, package_version, &serialized);

    // Add metadata about the schemas
    fingerprint
        .metadata
        .insert("schemas_count".to_string(), schemas.len().to_string());

    fingerprint.metadata.insert(
        "generation_source".to_string(),
        "compiled_schemas".to_string(),
    );

    Ok(fingerprint)
}

/// Check if cached data is still valid based on fingerprint
pub fn is_cache_valid(
    cached_fingerprint: &PackageFingerprint,
    current_fingerprint: &PackageFingerprint,
) -> bool {
    cached_fingerprint.matches(current_fingerprint)
}
